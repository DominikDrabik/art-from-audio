...
# --- Constants ---
AUDIO_EMBEDDING_DIM = 512  # CLAP embedding dimension
UNET_CROSS_ATTENTION_DIM = 768 # For Stable Diffusion 1.5 UNet
...
def get_clap_audio_embedding(audio_path, clap_model, clap_processor, device) -> torch.Tensor:
    ...
    # Ensure output is [1, embedding_dim] or [batch_size, embedding_dim]
    if audio_embeddings.ndim == 1:
        audio_embeddings = audio_embeddings.unsqueeze(0) # [dim] -> [1, dim]
    elif audio_embeddings.ndim == 3 and audio_embeddings.shape[1] == 1 : # [B, 1, dim] -> [B, dim]
        audio_embeddings = audio_embeddings.squeeze(1)
    return audio_embeddings # Expected shape [B, AUDIO_EMBEDDING_DIM]
...
def load_models(
    ...
    # Audio Projection - MLP to match training
    ...
    lora_config = LoraConfig(
        ...
        init_lora_weights="gaussian", # This doesn't matter for inference if weights are loaded
    ...
    state_dict = load_file(checkpoint_path, device="cpu") # Load to CPU first to avoid OOM on small GPUs
    ...
    return {
        ...
        "audio_projection": audio_projection, # This is the MLP
        "tokenizer": tokenizer, # For negative prompts
        "text_encoder": text_encoder, # For negative_prompts
        "model_dtype": model_dtype, # Store for autocast
    }
...
def generate_image_for_audio(audio_path: str, output_path: str, models: Dict[str, Any], args: Any):
    ...
    with torch.no_grad():
        ...
        # Expected shape [1, AUDIO_EMBEDDING_DIM] or [B, AUDIO_EMBEDDING_DIM]
        audio_embedding = get_clap_audio_embedding(audio_path, clap_model, clap_processor, DEVICE)
        audio_embedding = audio_embedding.to(dtype=audio_projection_mlp[0].weight.dtype) # Match MLP input layer dtype

        # 2. Project audio embedding using MLP
        # MLP expects [B, AUDIO_EMBEDDING_DIM]
        projected_embedding = audio_projection_mlp(audio_embedding) # Output shape [B, UNET_CROSS_ATTENTION_DIM]

        # 3. Prepare for UNet (expects [Batch, Seq_Len, Dim])
        # Here, Seq_Len is 1 for a single global audio embedding per image
        prompt_embeds = projected_embedding.unsqueeze(1).to(dtype=model_dtype) # -> [B, 1, UNET_CROSS_ATTENTION_DIM]

        # 4. Handle negative prompts (optional, for CFG)
        negative_prompt_embeds = None
        if args.guidance_scale > 1.0: # Classifier-Free Guidance
            # Create standard text conditioning for unconditional part
            tokens = [""] # Empty prompt for unconditional
            ...
            negative_prompt_embeds = uncond_embeds.to(dtype=model_dtype) # Shape [B, 77, UNET_CROSS_ATTENTION_DIM]
            
            # Ensure prompt_embeds has the same sequence length as negative_prompt_embeds
        ...
        with torch.amp.autocast(device_type=DEVICE, dtype=model_dtype if model_dtype != torch.float32 else None, enabled=(model_dtype != torch.float32)):
            image = pipeline(
                ...
                negative_prompt_embeds=negative_prompt_embeds, # Will be used if guidance_scale > 1
...
if __name__ == "__main__":
    ...
    if args.mixed_precision_inference == "no":
        args.mixed_precision_inference = None # load_models expects None for no mixed precision
