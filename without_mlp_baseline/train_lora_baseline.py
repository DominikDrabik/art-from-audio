    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if audio_embedding.ndim == 1 and audio_embedding.shape[0] == 512:
                ...
                print(f"Warning: Unexpected shape {audio_embedding.shape} for {embedding_path}")
                return None
...
def main():
    ...
    if audio_embedding_dim != unet_cross_attention_dim:
        audio_projection = nn.Linear(audio_embedding_dim, unet_cross_attention_dim).to(device)
    ...
    print(f"LoRA setup complete. Trainable LoRA parameters: {num_lora_params}")
    ...
    lr_scheduler = get_scheduler("cosine", optimizer=optimizer, num_warmup_steps=0, num_training_steps=max_steps)
    ...
    for epoch in range(NUM_TRAIN_EPOCHS):
        ...
        for step, batch in enumerate(dataloader):
            ...
            final_embeddings = audio_embeddings.to(target_dtype_inputs)
            if audio_projection:
                projected = audio_projection(audio_embeddings.float())
                final_embeddings = projected.to(dtype=target_dtype_inputs)
        ...
        progress_bar.set_postfix(avg_loss=epoch_loss_total / len(dataloader), lr=lr_scheduler.get_last_lr()[0])
        progress_bar.close()
    ...
    final_path = os.path.join(OUTPUT_DIR, "final_lora_and_projection_weights.safetensors")
